...
TEST(BaseQuantity, TestLocalizedUserInput)
{
    ...
    const Base::NumericLocaleContext enIn {
        "en_IN", ".", ",", "+", "-", 3, 2
    };

    EXPECT_EQ(
        Quantity::parseUserInput("12,345.67 mm", enUs),
        Quantity(12345.67, Unit::Length)
    );
    EXPECT_THROW(Quantity::parseUserInput("12,34,567 mm", enUs), ParserError);
    EXPECT_EQ(
        Quantity::parseUserInput("12,34,567 mm", enIn),
        Quantity(1234567, Unit::Length)
    );
}
