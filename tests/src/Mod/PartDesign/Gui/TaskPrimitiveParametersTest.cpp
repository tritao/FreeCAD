...
BlockingBoxState& getBlockingBoxState()
{
    static BlockingBoxState state;
    return state;
}
...
struct GuiHarness
{
        ...
        : app(true)
    {
        ...
        PartDesign::BlockingBoxTest::init();
...
PartDesignGui::TaskBoxPrimitives* findPrimitiveTaskBox(Gui::TaskView::TaskDialog* dialog)
{
    for (QWidget* widget : dialog->getDialogContent()) {
        if (auto* taskBox = qobject_cast<PartDesignGui::TaskBoxPrimitives*>(widget)) {
            return taskBox;
        }
    }
...
QPushButton* findCancelPreviewButton(PartDesignGui::TaskBoxPrimitives* taskBox)
{
...
{
    auto& state = getBlockingBoxState();
...
int getBlockingBoxTotalExecutionCount()
{
    auto& state = getBlockingBoxState();
    std::lock_guard<std::mutex> lock(state.mutex);
...
private Q_SLOTS:
    ...
    void init()  // NOLINT
    {
        ...
        primitiveView = dynamic_cast<PartDesignGui::ViewProviderPrimitive*>(guiDoc->getViewProvider(box));
    ...
    void primitiveAcceptWaitsForQueuedPreviewWithoutExtraRerun()  // NOLINT
    {
        ...
        QTRY_COMPARE_WITH_TIMEOUT(PartDesign::BlockingBoxTest::getExecutionCount(), 1, 3000);
        QCOMPARE(getBlockingBoxTotalExecutionCount(), 1);
        QVERIFY(taskBox->hasOutstandingRecompute());
        ...
        QCOMPARE(Gui::Control().activeDialog(doc), nullptr);
        QVERIFY(!guiDoc->hasPendingCommand());
        QCOMPARE(getBlockingBoxTotalExecutionCount(), 2);
    }
