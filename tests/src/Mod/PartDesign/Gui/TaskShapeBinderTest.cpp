...
struct BlockingShapeBinderState
{
    std::mutex mutex;
...
BlockingShapeBinderState& getBlockingShapeBinderState()
{
    static BlockingShapeBinderState state;
    return state;
}

void resetBlockingShapeBinderState()
{
    auto& state = getBlockingShapeBinderState();
...
void armBlockingShapeBinderState()
{
    auto& state = getBlockingShapeBinderState();
    std::lock_guard<std::mutex> lock(state.mutex);
    state.armed = true;
...
int getBlockingShapeBinderExecutionCount()
{
    auto& state = getBlockingShapeBinderState();
    std::lock_guard<std::mutex> lock(state.mutex);
...
int getBlockingShapeBinderTotalExecutionCount()
{
    auto& state = getBlockingShapeBinderState();
    std::lock_guard<std::mutex> lock(state.mutex);
...
void releaseBlockingShapeBinderState()
{
    auto& state = getBlockingShapeBinderState();
    {
        std::lock_guard<std::mutex> lock(state.mutex);
...
App::DocumentObjectExecReturn* executeBlockingShapeBinder(ExecuteBase&& executeBase)
{
    auto& state = getBlockingShapeBinderState();
    {
...
struct GuiHarness
{
        ...
        : app(true)
    {
        ...
        PartDesign::BlockingShapeBinderTest::init();
...
PROPERTY_SOURCE(PartDesign::BlockingShapeBinderTest, PartDesign::ShapeBinder)

void PartDesign::BlockingShapeBinderTest::resetBlocker()
{
    resetBlockingShapeBinderState();
}

void PartDesign::BlockingShapeBinderTest::armBlocker()
{
    armBlockingShapeBinderState();
}

int PartDesign::BlockingShapeBinderTest::getExecutionCount()
{
    return getBlockingShapeBinderExecutionCount();
}

void PartDesign::BlockingShapeBinderTest::releaseBlocker()
{
    releaseBlockingShapeBinderState();
}

App::DocumentObjectExecReturn* PartDesign::BlockingShapeBinderTest::execute()
{
    return executeBlockingShapeBinder([this]() { return PartDesign::ShapeBinder::execute(); });
}
...
private Q_SLOTS:
    ...
    void init()  // NOLINT
    {
        ...
        binderView = dynamic_cast<PartDesignGui::ViewProviderShapeBinder*>(guiDoc->getViewProvider(binder));
    ...
    void shapeBinderAcceptWaitsForQueuedPreviewWithoutExtraRerun()  // NOLINT
    {
        ...
        QTRY_COMPARE_WITH_TIMEOUT(PartDesign::BlockingShapeBinderTest::getExecutionCount(), 1, 3000);
        QCOMPARE(getBlockingShapeBinderTotalExecutionCount(), 1);
        QVERIFY(taskBox->hasOutstandingRecompute());
        ...
        QCOMPARE(Gui::Control().activeDialog(doc), nullptr);
        QVERIFY(!guiDoc->hasPendingCommand());
        QCOMPARE(getBlockingShapeBinderTotalExecutionCount(), 2);
