...
BlockingHoleState& getBlockingHoleState()
{
    static BlockingHoleState state;
    return state;
}

void resetBlockingHoleState()
{
    auto& state = getBlockingHoleState();
...
int getBlockingHoleExecutionCount()
{
    auto& state = getBlockingHoleState();
    std::lock_guard<std::mutex> lock(state.mutex);
...
int getBlockingHoleTotalExecutionCount()
{
    auto& state = getBlockingHoleState();
    std::lock_guard<std::mutex> lock(state.mutex);
...
App::DocumentObjectExecReturn* executeBlockingHole(ExecuteBase&& executeBase)
{
    auto& state = getBlockingHoleState();
    {
...
struct GuiHarness
{
        ...
        : app(true)
    {
        ...
        PartDesign::BlockingHoleTest::init();
...
PROPERTY_SOURCE(PartDesign::BlockingHoleTest, PartDesign::Hole)

void PartDesign::BlockingHoleTest::resetBlocker()
{
    resetBlockingHoleState();
}

void PartDesign::BlockingHoleTest::armBlocker()
{
    armBlockingHoleState();
}

int PartDesign::BlockingHoleTest::getExecutionCount()
{
    return getBlockingHoleExecutionCount();
}

void PartDesign::BlockingHoleTest::releaseBlocker()
{
    releaseBlockingHoleState();
}

App::DocumentObjectExecReturn* PartDesign::BlockingHoleTest::execute()
{
    return executeBlockingHole([this]() { return PartDesign::Hole::execute(); });
}
...
private Q_SLOTS:
    ...
    void holeAcceptWaitsForQueuedPreviewWithoutExtraRerun()  // NOLINT
    {
        ...
        QTRY_COMPARE_WITH_TIMEOUT(PartDesign::BlockingHoleTest::getExecutionCount(), 1, 3000);
        QCOMPARE(getBlockingHoleTotalExecutionCount(), 1);
        QVERIFY(taskBox->hasOutstandingRecompute());
        ...
        QCOMPARE(Gui::Control().activeDialog(doc), nullptr);
        QVERIFY(!guiDoc->hasPendingCommand());
        QCOMPARE(getBlockingHoleTotalExecutionCount(), 2);
    }
