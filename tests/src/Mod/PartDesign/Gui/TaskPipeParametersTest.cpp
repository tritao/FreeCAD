...
BlockingPipeState& getBlockingPipeState()
{
    static BlockingPipeState state;
    return state;
}

void resetBlockingPipeState()
{
    auto& state = getBlockingPipeState();
...
int getBlockingPipeExecutionCount()
{
    auto& state = getBlockingPipeState();
    std::lock_guard<std::mutex> lock(state.mutex);
...
int getBlockingPipeTotalExecutionCount()
{
    auto& state = getBlockingPipeState();
    std::lock_guard<std::mutex> lock(state.mutex);
...
App::DocumentObjectExecReturn* executeBlockingPipe(ExecuteBase&& executeBase)
{
    auto& state = getBlockingPipeState();
    {
...
struct GuiHarness
{
        ...
        : app(true)
    {
        ...
        PartDesign::BlockingAdditivePipeTest::init();
...
QComboBox* findOrientationModeCombo(PartDesignGui::TaskDlgPipeParameters* dialog)
{
    ...
    return orientationBox
        ? orientationBox->findChild<QComboBox*>(QStringLiteral("comboBoxMode"))
        : nullptr;
}
...
PROPERTY_SOURCE(PartDesign::BlockingAdditivePipeTest, PartDesign::AdditivePipe)

void PartDesign::BlockingAdditivePipeTest::resetBlocker()
{
    resetBlockingPipeState();
}

void PartDesign::BlockingAdditivePipeTest::armBlocker()
{
    armBlockingPipeState();
}

int PartDesign::BlockingAdditivePipeTest::getExecutionCount()
{
    return getBlockingPipeExecutionCount();
}

void PartDesign::BlockingAdditivePipeTest::releaseBlocker()
{
    releaseBlockingPipeState();
}

App::DocumentObjectExecReturn* PartDesign::BlockingAdditivePipeTest::execute()
{
    return executeBlockingPipe([this]() { return PartDesign::AdditivePipe::execute(); });
}
...
private Q_SLOTS:
    ...
    void init()  // NOLINT
    {
        ...
        Base::Interpreter().runString(
            ("import FreeCAD as App\n"
             ...
             "profile.addGeometry(Part.Circle(App.Vector(0, 0, 0), App.Vector(0, 0, 1), 1), False)\n"
        ...
        Base::Interpreter().runString(
            ("import FreeCAD as App\n"
             ...
             "spine.addGeometry(Part.LineSegment(App.Vector(0.0, 0.0, 0), App.Vector(0, 1, 0)), False)\n"
    ...
    void pipeAcceptWaitsForQueuedPreviewWithoutExtraRerun()  // NOLINT
    {
        ...
        QTRY_COMPARE_WITH_TIMEOUT(PartDesign::BlockingAdditivePipeTest::getExecutionCount(), 1, 3000);
        QCOMPARE(getBlockingPipeTotalExecutionCount(), 1);
        QVERIFY(taskBox->hasOutstandingRecompute());
        ...
        QCOMPARE(Gui::Control().activeDialog(doc), nullptr);
        QVERIFY(!guiDoc->hasPendingCommand());
        QCOMPARE(getBlockingPipeTotalExecutionCount(), 2);
    }
