...
BlockingDraftState& getBlockingDraftState()
{
    static BlockingDraftState state;
    return state;
}

void resetBlockingDraftState()
{
    auto& state = getBlockingDraftState();
...
int getBlockingDraftExecutionCount()
{
    auto& state = getBlockingDraftState();
    std::lock_guard<std::mutex> lock(state.mutex);
...
int getBlockingDraftTotalExecutionCount()
{
    auto& state = getBlockingDraftState();
    std::lock_guard<std::mutex> lock(state.mutex);
...
void releaseBlockingDraftState()
{
    auto& state = getBlockingDraftState();
    {
        std::lock_guard<std::mutex> lock(state.mutex);
...
App::DocumentObjectExecReturn* executeBlockingDraft(ExecuteBase&& executeBase)
{
    auto& state = getBlockingDraftState();
    {
...
struct GuiHarness
{
        ...
        : app(true)
    {
        ...
        PartDesign::BlockingDraftTest::init();
...
PROPERTY_SOURCE(PartDesign::BlockingDraftTest, PartDesign::Draft)

void PartDesign::BlockingDraftTest::resetBlocker()
{
    resetBlockingDraftState();
}

void PartDesign::BlockingDraftTest::armBlocker()
{
    armBlockingDraftState();
}

int PartDesign::BlockingDraftTest::getExecutionCount()
{
    return getBlockingDraftExecutionCount();
}

void PartDesign::BlockingDraftTest::releaseBlocker()
{
    releaseBlockingDraftState();
}

App::DocumentObjectExecReturn* PartDesign::BlockingDraftTest::execute()
{
    return executeBlockingDraft([this]() { return PartDesign::Draft::execute(); });
}
...
private Q_SLOTS:
    ...
    void draftAcceptWaitsForQueuedPreviewWithoutExtraRerun()  // NOLINT
    {
        ...
        QTRY_COMPARE_WITH_TIMEOUT(PartDesign::BlockingDraftTest::getExecutionCount(), 1, 3000);
        QCOMPARE(getBlockingDraftTotalExecutionCount(), 1);
        QVERIFY(taskBox->hasOutstandingRecompute());
        ...
        QCOMPARE(Gui::Control().activeDialog(doc), nullptr);
        QVERIFY(!guiDoc->hasPendingCommand());
        QCOMPARE(getBlockingDraftTotalExecutionCount(), 2);
    }
