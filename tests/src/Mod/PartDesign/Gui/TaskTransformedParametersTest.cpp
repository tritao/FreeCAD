...
struct BlockingTransformedState
{
    std::mutex mutex;
...
BlockingTransformedState& getBlockingTransformedState()
{
    static BlockingTransformedState state;
    return state;
}

void resetBlockingTransformedState()
{
    auto& state = getBlockingTransformedState();
...
void armBlockingTransformedState()
{
    auto& state = getBlockingTransformedState();
    std::lock_guard<std::mutex> lock(state.mutex);
    state.armed = true;
...
int getBlockingTransformedExecutionCount()
{
    auto& state = getBlockingTransformedState();
    std::lock_guard<std::mutex> lock(state.mutex);
...
void releaseBlockingTransformedState()
{
    auto& state = getBlockingTransformedState();
    {
        std::lock_guard<std::mutex> lock(state.mutex);
...
App::DocumentObjectExecReturn* executeBlockingTransformed(ExecuteBase&& executeBase)
{
    auto& state = getBlockingTransformedState();
    {
...
PartGui::PatternParametersWidget* findPrimaryPatternParametersWidget(
    QWidget* taskBox
)
{
    if (!taskBox) {
...
PROPERTY_SOURCE(PartDesign::BlockingLinearPatternTest, PartDesign::LinearPattern)
PROPERTY_SOURCE(PartDesign::BlockingPolarPatternTest, PartDesign::PolarPattern)
PROPERTY_SOURCE(PartDesign::BlockingMirroredTest, PartDesign::Mirrored)
PROPERTY_SOURCE(PartDesign::BlockingScaledTest, PartDesign::Scaled)
PROPERTY_SOURCE(PartDesign::BlockingMultiTransformTest, PartDesign::MultiTransform)

void PartDesign::BlockingLinearPatternTest::resetBlocker()
{
    resetBlockingTransformedState();
}

void PartDesign::BlockingLinearPatternTest::armBlocker()
{
    armBlockingTransformedState();
}

int PartDesign::BlockingLinearPatternTest::getExecutionCount()
{
    return getBlockingTransformedExecutionCount();
}

void PartDesign::BlockingLinearPatternTest::releaseBlocker()
{
    releaseBlockingTransformedState();
}

App::DocumentObjectExecReturn* PartDesign::BlockingLinearPatternTest::execute()
{
    return executeBlockingTransformed([this]() { return PartDesign::LinearPattern::execute(); });
}

void PartDesign::BlockingPolarPatternTest::resetBlocker()
{
    resetBlockingTransformedState();
}

void PartDesign::BlockingPolarPatternTest::armBlocker()
{
    armBlockingTransformedState();
}

int PartDesign::BlockingPolarPatternTest::getExecutionCount()
{
    return getBlockingTransformedExecutionCount();
}

void PartDesign::BlockingPolarPatternTest::releaseBlocker()
{
    releaseBlockingTransformedState();
}

App::DocumentObjectExecReturn* PartDesign::BlockingPolarPatternTest::execute()
{
    return executeBlockingTransformed([this]() { return PartDesign::PolarPattern::execute(); });
}

void PartDesign::BlockingMirroredTest::resetBlocker()
{
    resetBlockingTransformedState();
}

void PartDesign::BlockingMirroredTest::armBlocker()
{
    armBlockingTransformedState();
}

int PartDesign::BlockingMirroredTest::getExecutionCount()
{
    return getBlockingTransformedExecutionCount();
}

void PartDesign::BlockingMirroredTest::releaseBlocker()
{
    releaseBlockingTransformedState();
}

App::DocumentObjectExecReturn* PartDesign::BlockingMirroredTest::execute()
{
    return executeBlockingTransformed([this]() { return PartDesign::Mirrored::execute(); });
}

void PartDesign::BlockingScaledTest::resetBlocker()
{
    resetBlockingTransformedState();
}

void PartDesign::BlockingScaledTest::armBlocker()
{
    armBlockingTransformedState();
}

int PartDesign::BlockingScaledTest::getExecutionCount()
{
    return getBlockingTransformedExecutionCount();
}

void PartDesign::BlockingScaledTest::releaseBlocker()
{
    releaseBlockingTransformedState();
}

App::DocumentObjectExecReturn* PartDesign::BlockingScaledTest::execute()
{
    return executeBlockingTransformed([this]() { return PartDesign::Scaled::execute(); });
}

void PartDesign::BlockingMultiTransformTest::resetBlocker()
{
    resetBlockingTransformedState();
}

void PartDesign::BlockingMultiTransformTest::armBlocker()
{
    armBlockingTransformedState();
}

int PartDesign::BlockingMultiTransformTest::getExecutionCount()
{
    return getBlockingTransformedExecutionCount();
}

void PartDesign::BlockingMultiTransformTest::releaseBlocker()
{
    releaseBlockingTransformedState();
}

App::DocumentObjectExecReturn* PartDesign::BlockingMultiTransformTest::execute()
{
    return executeBlockingTransformed([this]() { return PartDesign::MultiTransform::execute(); });
}
    ...
    void prepareTransformedFixture(TransformedKind kind)
    {
        switch (kind) {
            case TransformedKind::Linear: {
                auto* pattern = doc->addObject<PartDesign::BlockingLinearPatternTest>("BlockingLinearPattern");
            ...
            case TransformedKind::Polar: {
                auto* pattern = doc->addObject<PartDesign::BlockingPolarPatternTest>("BlockingPolarPattern");
                QVERIFY(pattern != nullptr);
                body->addObject(pattern);
                pattern->Originals.setValues({baseBox});
            ...
            case TransformedKind::MultiTransformScaled: {
                ...
                auto* multiTransform =
                    doc->addObject<PartDesign::BlockingMultiTransformTest>("BlockingMultiTransform");
