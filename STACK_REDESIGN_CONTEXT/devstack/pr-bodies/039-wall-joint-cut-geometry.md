...
Expose resolved wall-joint seams and their endpoints as canonical semantic BIM
representation geometry.

# Why

Miter and tee boundaries were present in individual cut faces but were not
identified as relation-owned geometry. Coin could consequently obscure a seam
behind an adjacent wall fill, while snapping and documentation had to infer its
meaning from anonymous boundary edges.

# Architectural invariant

A resolved wall relation owns one renderer-neutral cut line and its semantic
cut points. Display, snapping, picking and TechDraw consume that same geometry
without reconstructing the joint independently.

# Changes

- Add `WallJointCutLine` projected geometry for every resolved wall-end trim.
- Add `WallJointCutPoint` snap targets mapped to both the wall and relation.
- Draw semantic seams above contextual wall fills.
- Convert renderer-neutral polyline geometry into shapes at the TechDraw
  adapter boundary.

# Tests

- Verify miter seams and both semantic endpoints in plan representations.
- Verify source identity includes the wall relation.
- Verify cut geometry remains on a rotated representation frame.
- Verify TechDraw accepts semantic polyline geometry directly.
- Run wall-joint, representation, SectionPlane and Plan Edit example suites.

<!-- AUTOGEN:BEGIN -->
### Patch Set

> [!IMPORTANT]
> Part `39/52` of a stacked series. Depends on `pr/unified-bim-plan-perfect/038-contextual-renderer-safety`; review and merge in order.

- Group: `representation-fidelity` — Semantic representation fidelity (`1/5`)
