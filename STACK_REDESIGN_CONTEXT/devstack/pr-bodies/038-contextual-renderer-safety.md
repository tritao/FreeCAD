...
Serialize Plan Edit's contextual Coin renderer mutations outside active viewer
event traversal.

# Why

Selection ray-picking could overlap a representation refresh or teardown and
observe a transient null child in a Coin `SoChildList`, causing a native crash.

# Architectural invariant

Semantic representation state may be computed synchronously, but every Coin
scene-graph mutation is coalesced onto the viewer event queue and guarded by the
owning renderer generation.

# Changes

- Queue representation refresh, removal, visibility, handle-state and teardown
  mutations.
- Coalesce repeated source refreshes.
- Invalidate stale callbacks when a renderer generation is replaced.
- Permit only explicit renderer finalizers during session shutdown.
- Restore Coin-managed camera clipping fields without changing saved-camera
  semantics.

# Tests

- Contextual renderer refreshes remain unchanged until the queue is flushed.
- Repeated refreshes coalesce by semantic source.
- Teardown is deferred safely and completes through a shutdown finalizer.
- The complete Plan Edit GUI session suite passes, including real Coin event
  activation, editing, visibility, undo and camera restoration.

<!-- AUTOGEN:BEGIN -->
### Patch Set

> [!IMPORTANT]
> Part `38/52` of a stacked series. Depends on `pr/unified-bim-plan-perfect/037-examples`; review and merge in order.

- Group: `stability` — Runtime stability (`1/1`)
