---
title: "BIM: add semantic opening geometry and handles"
---

## Summary

Makes hosted openings first-class contextual Plan representations with explicit jamb geometry, semantic snap points, canonical move/resize handles, and door action handles.

## Why

Plan Edit should edit the semantic opening directly through the same renderer-neutral representation used for display and snapping. Opening grips must not depend on a parallel Draft tracker path or GUI-only ViewProvider behavior.

## Architectural invariant

> Opening display, snapping, actions, and editing resolve through one `BIMRepresentation` owned by the semantic opening provider.

## Changes

- adds mapped opening jamb lines and jamb endpoint snap targets
- exposes canonical position and left/right jamb handles with distinct glyphs
- adds semantic door hinge and opening-direction actions outside the wall profile
- moves opening action semantics out of the GUI ViewProvider
- removes legacy Draft grip rendering and picking from the active Plan Edit path
- defers semantic commits until the originating Coin event has returned
- keeps host cut geometry current after anchored opening resizes
- adds headless representation assertions and real Coin move/resize/action coverage

## Compatibility

Existing explicit opening tools remain available. Selected-object grips are now realized and picked exclusively through the canonical contextual representation.

## Tests

- `bimtests.TestArchRepresentation` (20 tests)
- `bimtests.TestBimPlanEditSessionGui` (16 tests)
- `bimtests.TestBimPlanEditExamplesGui` (5 tests)
- pre-commit checks for all changed files

## Intentionally deferred

Operation-specific SVG/icon glyphs can replace the current semantic Cross/Plus action glyphs after the generic overlay icon contract is designed.

<!-- AUTOGEN:BEGIN -->
### Patch Set

> [!IMPORTANT]
> Part `40/52` of a stacked series. Depends on `pr/unified-bim-plan-perfect/039-wall-joint-cut-geometry`; review and merge in order.

- Group: `representation-fidelity` — Semantic representation fidelity (`2/5`)
