---
title: "Gui: render SVG icons as overlay glyphs"
---

## Summary

Extends the generic constant-pixel Coin overlay glyph with FreeCAD SVG icon rendering.

## Why

Discrete contextual actions need recognizable icons without introducing Qt widgets into individual 3D viewers.

## Architectural invariant

> FreeCAD GUI resolves and rasterizes icon resources; Coin owns their viewer-local placement, rendering, and picking.

## Changes

- adds the generic `ICON` glyph type and `iconName` field
- resolves SVGs through `BitmapFactory`
- rasterizes at the requested DPI-aware pixel size
- renders the result through a centered Coin `SoImage`
- recolors semantic SVG artwork for interaction states
- falls back to the existing marker glyph if resolution fails

## Compatibility

Existing geometric glyph behavior is unchanged.

## Tests

- builds the `FreeCADGui` target
- downstream BIM GUI coverage verifies populated Coin image nodes

## Intentionally deferred

A shared raster cache can be added if profiling shows it is necessary.

<!-- AUTOGEN:BEGIN -->
### Patch Set

> [!IMPORTANT]
> Part `41/52` of a stacked series. Depends on `pr/unified-bim-plan-perfect/040-opening-geometry-handles`; review and merge in order.

- Group: `representation-fidelity` — Semantic representation fidelity (`3/5`)
