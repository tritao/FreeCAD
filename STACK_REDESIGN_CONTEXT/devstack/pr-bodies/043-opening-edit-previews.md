---
title: "BIM: preview semantic opening edits"
---

## Summary

Previews hosted-opening move and resize operations through the same renderer-neutral representation contract used for their committed geometry.

## Why

An opening edit must show the future wall cut, jambs, and plan symbol before it mutates the BIM model. Invalid edits also need visible feedback without creating temporary document objects.

## Architectural invariant

> A contextual edit preview is a transient semantic representation of the proposed BIM state, not a mutation of the document or a Coin-only approximation.

## Changes

- adds optional semantic representation previews to edit operations
- generates proposed opening cut, jamb, symbol, and guide geometry without mutating the opening
- preserves the opposite jamb during one-sided resize previews
- realizes previews viewer-locally in blue, or red when validation rejects the proposal
- keeps preview geometry outside picking and semantic source mappings
- clears transient geometry on cancel, commit, and session teardown

## Compatibility

Existing shape previews remain supported for operations that do not provide a semantic representation preview.

## Tests

- renderer-neutral opening preview and non-mutation coverage
- real Coin move/resize/flip/undo/re-entry coverage with preview, cancel, invalid state, and commit parity
- Plan Edit session GUI regression suite
- pre-commit checks

## Intentionally deferred

Dimension readouts and richer constraint explanations can build on the same preview result later.

<!-- AUTOGEN:BEGIN -->
### Patch Set

> [!IMPORTANT]
> Part `43/52` of a stacked series. Depends on `pr/unified-bim-plan-perfect/042-opening-action-icons`; review and merge in order.

- Group: `representation-fidelity` — Semantic representation fidelity (`5/5`)
