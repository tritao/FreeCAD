---
title: "Gui: add screen-space overlay glyphs"
---
## Summary

Adds a reusable Coin scene-graph node for small, screen-space interaction glyphs anchored at 3D model positions.

## Why

Interactive handles need stable pixel sizing, explicit shapes, and overlay rendering independent of model depth. Keeping that behavior in one generic GUI node avoids each workbench assembling its own fragile Coin subtree.

## Architectural invariant

The GUI owns glyph realization and render ordering; consumers supply only a world position, visual role, color, and pixel size.

## Changes

* Adds `SoFCOverlayGlyph` with circle, square, diamond, cross, and plus shapes.
* Uses FreeCAD's existing marker bitmap inventory and selects the nearest supported pixel size.
* Renders through an annotation branch with unlit base color so handles remain legible and in front of model geometry.
* Registers the node with FreeCAD's Coin type system for C++ and Python consumers.

## Compatibility

This is an additive generic GUI primitive. It does not change existing selection, marker, or ViewProvider behavior.

## Tests

The BIM Plan Edit GUI tests instantiate the registered node through Coin, verify its type, and exercise it as the semantic-handle renderer. The complete GUI target was rebuilt after registration changes.

## Intentionally deferred

Picking policy and BIM handle semantics remain consumer responsibilities. This layer provides only renderer-side glyph realization.

<!-- AUTOGEN:BEGIN -->
### Patch Set

> [!IMPORTANT]
> Part `16/52` of a stacked series. Depends on `pr/unified-bim-plan-perfect/015-viewer-representation-instances`; review and merge in order.

- Group: `contextual-views` — Contextual views and viewer-local realization (`10/10`)
