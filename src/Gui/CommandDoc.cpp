...
protected:
    ...
    void pasteExpressions()
    {
        ...
        for (; headerPos != std::string_view::npos;) {
            ...
            size_t len = (nextHeaderPos == std::string_view::npos ? view.size() : nextHeaderPos)
                - exprStart;
            try {
                App::ExpressionPtr expr(
                    App::Expression::parse(obj, std::string(view.substr(exprStart, len)))
                );
