...
constexpr std::array<SoNaviCube::PickId, 8> kButtonPickIds = {
...
void SoNaviCube::addButtonFace(PickId pickId) const
{
    ...
    // polygon interpretation. (ViewMenu is a multi-triangle icon encoded with repeated vertices,
    // so it is handled explicitly.)
    std::vector<int> tris;
    if (pickId == PickId::ViewMenu) {
        // The point list encodes three quads (each closed by repeating the first vertex).
        // Triangulate each quad independently.
        //  - Face 1: 0-1-2-3
        //  - Face 2: 4-5-6-7
        //  - Face 3: 8-9-10-11
        tris = {
    ...
    if (!tris.empty()) {
        buttonTriangleIndices[pickIndex(pickId)] = std::move(tris);
    }
    else {
