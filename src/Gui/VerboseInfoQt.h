...
#ifdef FC_OS_MACOSX
    auto macosVersionFile = QStringLiteral("/System/Library/CoreServices/.SystemVersionPlatform.plist");
...
{
    const QString deskEnv = QProcessEnvironment::systemEnvironment().value(
        QStringLiteral("XDG_CURRENT_DESKTOP"),
        QString()
    );
    const QString deskSess = QProcessEnvironment::systemEnvironment().value(
...
# endif
    ...
    QString navStyle = QString::fromStdString(
        hGrp->GetASCII("NavigationStyle", "Gui::CADNavigationStyle")
    );
