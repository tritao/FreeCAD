    ...
    , d_ptr(new QuantitySpinBoxPrivate(this))
{
    ...
    connect(this, &QuantitySpinBox::editingFinished, this, [&] { this->validateInput(); });
}
...
void QuantitySpinBox::validateInput()
{
    ...
    const auto result = d->interpretInput(text, path, App::InputPhase::Commit);
    ...
    switch (result.diagnostic->kind) {
        case App::InputDiagnosticKind::IncompleteNumber:
            message = tr("Incomplete number");
            break;
        case App::InputDiagnosticKind::MalformedGrouping:
            message = tr("Malformed grouping separator placement");
            break;
        case App::InputDiagnosticKind::ExpressionSyntax:
            message = tr("Invalid expression");
            break;
        case App::InputDiagnosticKind::Evaluation:
            message = tr("Expression could not be evaluated");
            break;
        case App::InputDiagnosticKind::IncompatibleUnit:
            message = tr("Incompatible unit");
            break;
        case App::InputDiagnosticKind::OutOfRange:
            message = tr("Value is outside the allowed range");
            break;
    }

    showIcon();
    iconLabel->setToolTip(message);
    lineEdit()->setToolTip(message);
    const auto& diagnostic = *result.diagnostic;
    const QByteArray utf8 = text.toUtf8();
    const auto offset = qMin<int>(
        static_cast<int>(diagnostic.offset), utf8.size()
    );
    const auto length = qMin<int>(
        static_cast<int>(diagnostic.length), utf8.size() - offset
    );
    ...
    QToolTip::showText(
        lineEdit()->mapToGlobal(QPoint(0, lineEdit()->height())), message, lineEdit()
    );
...
void QuantitySpinBox::userInput(const QString& text)
{
    Q_D(QuantitySpinBox);
    const App::ObjectIdentifier& path = getPath();
    const auto result = d->interpretInput(text, path, App::InputPhase::Editing);
    d->lastRejectedText.clear();
    QToolTip::hideText();
