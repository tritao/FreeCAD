...
bool MayaGestureNavigationStyle::hasPrimaryButton(const NavigationInputState& input)
{
    return input.left;
}

bool MayaGestureNavigationStyle::hasSecondaryButton(const NavigationInputState& input)
{
    return input.right;
}
...
{
    auto* data = std::get_if<AwaitingMoveData>(&stateData);
    if (!data) {
        return;
    }
...
void MayaGestureNavigationStyle::enterAwaitingMove(const EventContext& context)
{
    ...
    data.deferredEvents.push_back(*context.event.mouseButton());
    stateData = std::move(data);
    state = State::AwaitingMove;
    setupPanningPlane(viewer->getSoRenderManager()->getCamera());
}

void MayaGestureNavigationStyle::enterMotion(State next, const SoEvent* event)
{
    MotionData data;
...
void MayaGestureNavigationStyle::enterGesture(GestureKind kind, const SoEvent* event)
{
    stateData = GestureData {kind};
    state = State::Gesture;
...
{
    if (const auto* data = std::get_if<AwaitingMoveData>(&stateData)) {
        for (const SoMouseButtonEvent& event : data->deferredEvents) {
...
{
    if (auto* data = std::get_if<AwaitingMoveData>(&stateData)) {
        data->deferredEvents.clear();
    }
}

bool MayaGestureNavigationStyle::hasDeferredEvents() const
{
    const auto* data = std::get_if<AwaitingMoveData>(&stateData);
    return data && !data->deferredEvents.empty();
}

MayaGestureNavigationStyle::EventOutcome MayaGestureNavigationStyle::handleIdle(
...
MayaGestureNavigationStyle::EventOutcome MayaGestureNavigationStyle::handleAwaitingMoveButton(
    const EventContext& context
)
{
    const NavigationEventView& event = context.event;
    ...
    if (button == SoMouseButtonEvent::BUTTON1 || button == SoMouseButtonEvent::BUTTON2) {
        if (press) {
            ...
            if (state == State::Idle) {
                ...
                data.deferredEvents.push_back(*mouse);
            }
            return {.processed = true};
        }

        const auto* data = std::get_if<AwaitingMoveData>(&stateData);
        const bool complexClick = data && data->complexClick;
        if (button == SoMouseButtonEvent::BUTTON2 && !complexClick && !context.editing
            && isPopupMenuEnabled()) {
            clearDeferredEvents();
            leaveState();
    ...
    if (button == SoMouseButtonEvent::BUTTON3) {
        if (press && context.event.input().alt) {
            ...
            enterMotion(State::Pan, event.event());
        }
        else if (press) {
            setupPanningPlane(viewer->getCamera());
...
MayaGestureNavigationStyle::EventOutcome MayaGestureNavigationStyle::handleAwaitingMoveMotion(
    const EventContext& context
)
{
    auto& data = std::get<AwaitingMoveData>(stateData);
    ...
    if (data.moveThresholdBroken && hasDeferredEvents()) {
        ...
        if ((input.left && !suppressLmbDrag && input.alt) || (input.right && input.alt)) {
            ...
            enterMotion(input.left ? State::Rotate : State::Zoom, context.event.event());
            return {.processed = true};
        }
    ...
    return hasDeferredEvents() ? EventOutcome {.processed = true} : EventOutcome {};
}

MayaGestureNavigationStyle::EventOutcome MayaGestureNavigationStyle::handleAwaitingMoveGesture(
    const EventContext& context
)
{
    const NavigationEventView& event = context.event;
    ...
    if (event.event()->isOfType(SoGesturePanEvent::getClassTypeId())) {
        setupPanningPlane(viewer->getSoRenderManager()->getCamera());
        enterGesture(GestureKind::Pan, event.event());
        return {.processed = true};
    }
    if (event.event()->isOfType(SoGesturePinchEvent::getClassTypeId())) {
        setupPanningPlane(viewer->getSoRenderManager()->getCamera());
        enterGesture(GestureKind::Pinch, event.event());
        return {.processed = true};
    }
...
MayaGestureNavigationStyle::EventOutcome MayaGestureNavigationStyle::handleAwaitingMove(
    const EventContext& context
)
{
...
MayaGestureNavigationStyle::EventOutcome MayaGestureNavigationStyle::handleMotionButton(
    const EventContext& context
)
{
    ...
    if (hasPrimaryButton(input) || hasSecondaryButton(input)) {
        if (hasPrimaryButton(input) && hasSecondaryButton(input)) {
            setRotationCenter(viewer->getFocalPoint());
            enterMotion(State::Rotate, context.event.event());
        }
        else if (hasPrimaryButton(input)) {
            saveCursorPosition(context.event.event());
            enterMotion(State::Rotate, context.event.event());
        }
        else {
            saveCursorPosition(context.event.event());
            enterMotion(State::Pan, context.event.event());
        }
    }
    else {
        leaveState();
...
MayaGestureNavigationStyle::EventOutcome MayaGestureNavigationStyle::handleMotionPointer(
    const EventContext& context
)
{
    if (state == State::Zoom) {
        ...
        if (hasPrimaryButton(context.event.input()) && hasSecondaryButton(context.event.input())) {
...
MayaGestureNavigationStyle::EventOutcome MayaGestureNavigationStyle::handleMotionGesture(
    const EventContext& context
)
{
...
MayaGestureNavigationStyle::EventOutcome MayaGestureNavigationStyle::handleMotion(
    const EventContext& context
)
{
    if (context.event.isMouseButton()) {
    ...
    if (context.event.isGesture()) {
        return handleMotionGesture(context);
    }
    return {};
}

MayaGestureNavigationStyle::EventOutcome MayaGestureNavigationStyle::handleGesture(
    const EventContext& context
)
{
    if (context.event.isMouseButton()) {
    ...
    const auto* data = std::get_if<GestureData>(&stateData);
    if (!data) {
        return {.processed = true};
    }

    if (data->kind == GestureKind::Pinch
        && context.event.event()->isOfType(SoGesturePinchEvent::getClassTypeId())) {
        ...
        data->kind == GestureKind::Pan
        && context.event.event()->isOfType(SoGesturePanEvent::getClassTypeId())
    ) {
...
MayaGestureNavigationStyle::EventOutcome MayaGestureNavigationStyle::dispatchEvent(
    const EventContext& context
)
{
    switch (state) {
...
SbBool MayaGestureNavigationStyle::processSoEvent(const SoEvent* const event)
{
    ...
    if (isButton) {
        const auto* mouse = static_cast<const SoMouseButtonEvent*>(event);
        const bool press = mouse->getState() == SoButtonEvent::DOWN;
    ...
    EventOutcome outcome = dispatchEvent(context);
