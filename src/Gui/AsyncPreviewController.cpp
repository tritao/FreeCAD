...
void AsyncPreviewController::requestRecompute(bool waitForCompletion)
{
    ...
    const bool canRunOnWorker = asyncEnabled && App::GetApplication().canRecomputeRequestOnWorker(request);
    {
        std::ostringstream details;
        details << describeRecomputeRequest(request);
...
)
{
    ...
    const bool clampedRegression =
        state.determinate && !generationChanged && state.percent < recomputeProgressPercent;
    ...
    if (generationChanged || clampedRegression || textChanged || percentChanged || determinateChanged) {
        std::ostringstream details;
        details << "generation=" << generation;
        ...
        details << " display_text=" << (recomputeProgressText.empty() ? "<empty>" : recomputeProgressText);
