...
void applyOverlay(SoNode* root, int width, int height)
{
...
void GLPainter::drawRect(int x1, int y1, int x2, int y2)
{
    ...
    applyOverlay(root, width, height);
    root->unref();
}
...
void Polyline::paintGL()
{
    ...
    if (closed && !stippled) {
        ...
        if (closed && stippled) {
            ...
            const QPoint& first = _cNodeVector.front();
            closeVp->vertex.set1Value(0, toOverlayPoint(last.x(), last.y(), viewportWidth, viewportHeight));
            closeVp->vertex.set1Value(1, toOverlayPoint(first.x(), first.y(), viewportWidth, viewportHeight));
