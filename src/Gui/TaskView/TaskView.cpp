...
{
    auto* manager = Gui::WorkbenchManager::instance();
    return formatWorkbenchLabel(manager ? manager->activeName() : std::string());
}
...
void TaskView::updateWatcherContext(int visibleSections)
{
    ...
    if (!App::GetApplication().getActiveDocument()) {
        ...
        watcherHintLabel->setText(
            tr("No actions match the current context yet. Select objects or start a command from the toolbar.")
        );
    }
    else {
        watcherHintLabel->setText(
            tr("No actions match the current selection. Try a different selection or start a command "
               "from the toolbar.")
        );
    }
}
...
void TaskView::updateWatcherEmptyState(int visibleSections)
{
    ...
    if (!App::GetApplication().getActiveDocument()) {
        ...
        watcherEmptyBodyLabel->setText(
            tr("Open an existing document or create a new one to populate this panel with contextual actions.")
        );
    }
    else if (Gui::Selection().size() == 0) {
        watcherEmptyTitleLabel->setText(tr("No Contextual Actions Yet"));
        watcherEmptyBodyLabel->setText(
            tr("This workbench does not expose a starter action for the current state. Select geometry "
               "or use a toolbar or menu command to begin a task.")
        );
    }
    else {
        watcherEmptyTitleLabel->setText(tr("No Matching Actions"));
        watcherEmptyBodyLabel->setText(
            tr("The current selection does not match any contextual action group. Adjust the selection "
               "or start a command directly.")
        );
    }
}
