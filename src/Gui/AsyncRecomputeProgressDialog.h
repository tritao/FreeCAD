...
AsyncRecomputeProgressOutcome runSyncDocumentRecomputeFallback(
...
)
{
    ...
    QObject::connect(
        ...
        &controller,
        [&controller]() { controller.stopPendingRecompute(); }
    );
    QObject::connect(
        &controller,
    ...
    QObject::connect(
        ...
        &QTimer::timeout,
        &dialog,
        [&dialog, &controller, fallbackLabel]() {
            if (!controller.hasOutstandingRecompute()) {
                return;
            }
            updateAsyncRecomputeProgressDialog(dialog, controller, fallbackLabel);
...
)
{
    ...
    callbacks.makeRequest = [recomputeRequest]() { return recomputeRequest; };
    callbacks.runSync = std::forward<RunSyncFn>(runSync);
    callbacks.onCompleted =
        [&outcome](bool success, bool canceled, App::RecomputeFailure failure, const std::string& message) {
