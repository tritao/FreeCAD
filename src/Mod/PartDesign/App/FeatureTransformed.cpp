...
App::DocumentObjectExecReturn* Transformed::execute()
{
    ...
    auto getTransformedCompShape =
        ...
        transformIter++;
        const std::size_t totalTransformations =
            transformations.size() > 1 ? transformations.size() - 1 : 0;
        std::size_t transformIndex = 0;
        for (; transformIter != transformations.end(); transformIter++, transformIndex++) {
            auto transformScope = scope.makeStepScope(
                transformIndex,
    ...
    try {
        switch (mode) {
            case Mode::Features:
                ...
                for (std::size_t originalIndex = 0; originalIndex < originals.size(); ++originalIndex) {
                ...
                appendTransformedDebugLog(
                    *this,
                    ...
                    std::string("elapsed_ms=")
                        + std::to_string(elapsedMilliseconds(wholeFuseStart))
                );
