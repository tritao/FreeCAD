...
void reportHoleAsyncRecomputeFailure(
...
)
{
    ...
    auto report = [message]() { Base::Console().error("%s\n", message.c_str()); };
    ...
    , ui(new Ui_TaskHoleParameters)
{
    ...
    callbacks.runSync = [this]() { recomputeFeature(); };
...
void TaskHoleParameters::refreshThreadEnumCombos()
{
    ...
    auto updateComboBoxItems = [this](QComboBox* widget, const auto& values, int selected, bool translate) {
    ...
    updateComboBoxItems(ui->ThreadSize, hole->ThreadSize.getEnumVector(), hole->ThreadSize.getValue(), false);
    updateComboBoxItems(ui->ThreadClass, hole->ThreadClass.getEnumVector(), hole->ThreadClass.getValue(), true);
    updateComboBoxItems(ui->HoleCutType, hole->HoleCutType.getEnumVector(), hole->HoleCutType.getValue(), true);
}
