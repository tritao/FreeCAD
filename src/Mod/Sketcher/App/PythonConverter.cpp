...
)
{
    ...
    for (auto geo : geos) {
        ...
        if (sg.construction) {
            geolist
                = fmt::sprintf("%sconstrGeoList.append(%s)\n", geolist.c_str(), sg.creation.c_str());
        }
        else {
...
)
{
    ...
    for (auto constraint : constraints) {
        ...
        constraintlist
            = fmt::sprintf("%s\nconstraintList.append(%s)", constraintlist.c_str(), cg.c_str());
    }

    if (!constraints.empty()) {
        constraintlist = fmt::sprintf(
...
std::string makeSplineInfoArrayString(const std::vector<T>& rInfoVec)
{
    ...
    if constexpr (std::is_same_v<T, Base::Vector3d>) {
        for (const auto& rInfo : rInfoVec) {
            stream << "App.Vector(" << fmt::sprintf("%.8f", rInfo.x) << ", "
                   << fmt::sprintf("%.8f", rInfo.y) << "), ";
        }
    }
    else {
        for (const auto& rInfo : rInfoVec) {
            stream << fmt::sprintf("%.8f", rInfo) << ", ";
        }
    }
    ...
    static std::map<const Base::Type, std::function<SingleGeometry(const Part::Geometry* geo)>> converterMap = {
        ...
        {Part::GeomPoint::getClassTypeId(),
         [](const Part::Geometry* geo) {
             ...
             SingleGeometry sg;
             sg.creation = fmt::sprintf(
                 "Part.Point(App.Vector(%.8f, %.8f, %.8f))",
                 sgeo->getPoint().x,
                 sgeo->getPoint().y,
                 sgeo->getPoint().z
             );
        ...
        {Part::GeomBSplineCurve::getClassTypeId(),
         [](const Part::Geometry* geo) {
             ...
             SingleGeometry sg;
             sg.creation = fmt::sprintf(
                 "Part.BSplineCurve(%s, %s, %s, %s, %d, %s, False)",
                 controlpoints.c_str(),
                 mults.c_str(),
                 knots.c_str(),
