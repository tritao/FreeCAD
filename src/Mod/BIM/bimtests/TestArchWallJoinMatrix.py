...
class TestArchWallJoinMatrix(TestArchBase.TestArchBase):
    ...
    def _make_wall_between(self, baseline_kind, p1, p2, width=200.0, height=1500.0):
        ...
        self.fail(f"Unsupported baseline kind in test matrix: {baseline_kind}")
    ...
    def test_wall_join_matrix_explicit_tee_stem_selection(self):
        self.printTestMessage("Testing matrix coverage for explicit tee stem selection...")

        baseline_kinds = ("baseless", "line", "sketch")

        for baseline_kind in baseline_kinds:
            for tee_stem, expected_trimmed_a, expected_trimmed_b in (
                ("WallA", True, False),
                ("WallB", False, True),
            ):
                with self.subTest(baseline=baseline_kind, tee_stem=tee_stem):
                    existing_names = {obj.Name for obj in self.document.Objects}
                    wall_a = self._make_wall_between(
                        baseline_kind,
                        App.Vector(0, 0, 0),
                    ...
                    joint = Arch.makeWallJoint(wall_a, wall_b, "Tee")
                    joint.TeeStem = tee_stem
                    self.document.recompute()

                    self.assertEqual(joint.Status, "OK")
                    self.assertEqual(joint.TeeStem, tee_stem)
                    self._assert_manual_endings_clear(wall_a, wall_b)
                    self._assert_wall_trim_state(
                        wall_a,
                        initial_volume_a,
                        expected_trimmed_a,
                        "Explicit tee-stem selection should control the trim on wall A.",
                    )
                    self._assert_wall_trim_state(
                        wall_b,
                        initial_volume_b,
                        expected_trimmed_b,
                        "Explicit tee-stem selection should control the trim on wall B.",
                    )
