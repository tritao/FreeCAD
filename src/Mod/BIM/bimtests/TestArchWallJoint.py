...
class TestArchWallJoint(TestArchBase.TestArchBase):
    ...
    def test_miter_plan_representation_exposes_semantic_corner_boundaries(self):
        ...
        for wall in (wall1, wall2):
            ...
            segments = zip(boundaries[0], boundaries[0][1:])
            self.assertTrue(
                any(
                    abs(end.x - start.x) > 1e-7 and abs(end.y - start.y) > 1e-7
    ...
    def test_miter_respects_mixed_wall_section_alignment(self):
        ...
        wall1 = self._make_baseless_wall_between(
            App.Vector(0, 0, 0), App.Vector(1000, 0, 0)
        )
        wall1.Align = "Left"
        ...
        self._assert_miter_walls_closed(wall1, wall2)
        self.assertAlmostEqual(
            wall1.Shape.common(wall2.Shape).Volume,
            0.0,
            delta=1e-6,
            msg="Offset-aware mitered walls must meet without overlapping volumes.",
        )
