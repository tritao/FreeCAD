...
class BimPlanEditGuiWallsMixin:
    ...
    def _get_displayed_opening_symbol_points(self, opening):
        proxy = opening.ViewObject.Proxy
        points = proxy.lcoords.point
        displayed = []
        for index in range(points.getNum()):
            point = points[index]
            displayed.append(FreeCAD.Vector(point[0], point[1], point[2]))
        self.assertTrue(displayed)
        return displayed

    def _get_computed_opening_symbol_points(self, opening):
        points = []
        for polyline in opening.ViewObject.Proxy._collect_local_footprint_polylines() or []:
            for point in polyline:
                points.append(FreeCAD.Vector(point))
        self.assertTrue(points)
        return points

    def _normalize_opening_symbol_points(self, points):
        return sorted(
            (
                round(point.x, 2),
                round(point.y, 2),
    ...
    def test_plan_edit_t2_symbols_wall_resize_refreshes_opening_display_without_space_warning(self):
        ...
        space = self.document.getObject("Space003")
        self.assertIsNotNone(wall)
        self.assertIsNotNone(opening)
        self.assertIsNotNone(space)
        self.assertEqual(space.Label, "Bedroom001")
        ...
        computed_before = self._get_computed_opening_symbol_points(opening)
        self.assertEqual(
            self._normalize_opening_symbol_points(display_before),
            self._normalize_opening_symbol_points(computed_before),
        )
        ...
        initial_start, initial_end = self._get_wall_endpoints(wall)
        axis = initial_end.sub(initial_start)
        self.assertGreater(axis.Length, 0.0)
        axis.normalize()
        new_start = initial_start.add(axis.multiply(900.0))
        ...
        display_after = self._get_displayed_opening_symbol_points(opening)
        computed_after = self._get_computed_opening_symbol_points(opening)
        self.assertNotEqual(
            self._normalize_opening_symbol_points(computed_before),
            self._normalize_opening_symbol_points(computed_after),
        )
        self.assertEqual(
            self._normalize_opening_symbol_points(display_after),
            self._normalize_opening_symbol_points(computed_after),
        )
        ...
        self.assertNotIn("closed room loop", error_output)
    ...
    def test_plan_edit_deferred_host_shape_change_clears_selected_wall(self):
        """Deferred hosted-opening changes should not leave the previous wall grips selected."""
    ...
    def test_plan_edit_deferred_host_shape_change_keeps_new_wall_selection(self):
        """Deferred wall invalidation should not clear a newer wall selection."""
        ...
        FreeCADGui.Selection.clearSelection()
        FreeCADGui.Selection.addSelection(wall_a)
        self.pump_gui_events()
        ...
        with session.document_visuals.defer_document_visual_updates():
            ...
            self.pump_gui_events()
            session.selection.refresh.refresh_primary_selected_plan_target()
            self._assert_selected_wall_visuals(session, wall_b)
