...
def set_selected_space_label(session, label):
    space = _selection_state_api(session).get_selected_plan_target_object("space")
...
def set_selected_region_label(session, label):
    region = _selection_state_api(session).get_selected_plan_target_object("region")
    if not _selection_targets_api(session).is_plan_region_object(region):
        return False
    label = str(label or "").strip()
...
def set_selected_region_parent_space(session, space):
    ...
    if space is not None and not _selection_targets_api(session).is_plan_space_object(space):
        return False
...
def restore_selected_semantic_target(session, kind, obj, *, clear_edit_space=False):
    ...
    if obj:
        _selection_state_api(session).set_selected_plan_target(kind, obj, pending_restore=True)
        _selection_sync_api(session).set_gui_selection_object(obj)
    else:
        _selection_state_api(session).set_selected_plan_target()
    sync_method()
    session.task_panels.refresh_task_panel_status()
