...
def rehost_object(obj, host, *, preserve_world_position=False):
    """Assign a Host/Hosts link while optionally preserving the world pose."""

    if not can_rehost_object(obj, host):
        return False
    snapshot = _snapshot_pose(obj) if preserve_world_position else None
    try:
        if hasattr(obj, "Host"):
            obj.Host = host
        else:
            obj.Hosts = [host] if host is not None else []
        _restore_pose(obj, snapshot)
    except Exception:
        return False
    return True
