...
def selection_observer_set_preselection(session, doc, obj, sub):
    with _trace_selection_observer_event(
        ...
        queue_clear_gui_preselection(session)
...
def queue_clear_gui_preselection(session):
    """Clear preselection after the current Coin selection action returns."""

    state = session.selection_sync_state
    if state.clear_gui_preselection_queued:
        return
    try:
        from PySide import QtCore
    except ImportError:
        ...
        return
    state.clear_gui_preselection_queued = True

    def flush():
        state.clear_gui_preselection_queued = False
