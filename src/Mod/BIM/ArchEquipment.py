...
class _Equipment(ArchComponent.Component):
    ...
    def getFootprint(self, obj):
        """Return plan footprint faces for equipment.

        Prefer authored plan symbols when present. Otherwise use a horizontal
        slice through the equipment at the standard plan cut height. Shorter
        equipment is sliced just below its top and flattened to its base
        elevation so low furniture still appears in plan. If no slice can be
        built, fall back to literal bottom faces.
        """

        symbol_shapes = list(self._iter_plan_symbol_shapes(obj))
        symbol_faces = []
