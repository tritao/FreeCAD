...
class Arch_Window:
    ...
    def Activated(self):
        ...
        if session and self.sel:
            try:
                selected = session.selection.state.get_selected_plan_target_object("wall")
                if selected and self.sel[0] == selected:
                    session.selection.refresh.suspend_selected_wall_state(selected)
            except Exception:
                pass
    ...
    def getPoint(self, point=None, obj=None):
        ...
        session = BimPlanSession.get_active_session()
        defer_updates = (
            session.defer_document_visual_updates()
            if session and hasattr(session, "defer_document_visual_updates")
            else nullcontext()
        )
        with defer_updates:
            self.doc.openTransaction(translate("Arch", "Create Window"))
