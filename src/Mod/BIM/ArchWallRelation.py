...
def solve_wall_joint_inputs(
    ...
    if joint_type == "Miter":
        plane_a, plane_b = calculate_miter_cutting_planes(
            path_a, path_b, section_a, section_b, intersection
        )
...
def calculate_miter_cutting_planes(path1, path2, section1, section2, intersection):
    """Calculate offset-aware global cutting planes for a miter joint.

    A bisector through the baseline intersection is sufficient only when both
    wall sections are symmetric and equally aligned.  The actual architectural
    seam joins the intersection of the two inside faces to the intersection of
    the two outside faces.  Deriving the plane from those section boundaries
    keeps mixed widths, offsets, and Left/Right alignments closed.
    """
    dir1 = path1.direction_away_from(intersection)
    dir2 = path2.direction_away_from(intersection)
    lateral1 = path1.lateral_direction()
    lateral2 = path2.lateral_direction()
    offsets1 = (-section1.y_max, -section1.y_min)
    offsets2 = (-section2.y_max, -section2.y_min)
    toward1 = offsets1[1] if lateral1.dot(dir2) >= 0 else offsets1[0]
    toward2 = offsets2[1] if lateral2.dot(dir1) >= 0 else offsets2[0]
    away1 = offsets1[0] if lateral1.dot(dir2) >= 0 else offsets1[1]
    away2 = offsets2[0] if lateral2.dot(dir1) >= 0 else offsets2[1]

    inside = _intersect_offset_paths(
        intersection + lateral1 * toward1,
        dir1,
        intersection + lateral2 * toward2,
        dir2,
        path1.normal,
    )
    outside = _intersect_offset_paths(
        intersection + lateral1 * away1,
        dir1,
        intersection + lateral2 * away2,
        dir2,
        path1.normal,
    )
    ...
    axis_x = seam
    axis_y = path1.normal
    axis_z = seam.cross(axis_y).normalize()
    rotation = FreeCAD.Rotation(axis_x, axis_y, axis_z, "ZXY")
    plane1 = FreeCAD.Placement(inside, rotation)
    plane2 = plane1.copy()
    plane2.rotate(inside, path1.normal, 180)
    return plane1, plane2


def _intersect_offset_paths(point1, direction1, point2, direction2, normal):
    """Return the intersection of two coplanar offset path lines."""
    denominator = direction1.cross(direction2).dot(normal)
    if abs(denominator) <= 1e-9:
        return None
    parameter = (point2 - point1).cross(direction2).dot(normal) / denominator
    return point1 + direction1 * parameter
