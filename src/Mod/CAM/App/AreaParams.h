...
/** clipper fill type */
#define AREA_CLIPPER_FILL_SEQ (NonZero, EvenOdd, Positive, Negative)
#define AREA_CLIPPER_FILL_INFO (ClipperLib::PolyFillType, ClipperLib::pft)

/** Parameters of clipper fill types */
#define AREA_PARAMS_CLIPPER_FILL(_apply, _tail, ...) \
    _apply(__VA_ARGS__, FC_PP_IF(_tail, 1, 0), (enum2, subject_fill, SubjectFill, 0, \
                                                "ClipperLib subject fill type. \nSee https://goo.gl/5pYQQP", \
                                                AREA_CLIPPER_FILL_SEQ, AREA_CLIPPER_FILL_INFO)) \
    _apply(__VA_ARGS__, 1, (enum2, clip_fill, ClipFill, 0, \
                            "ClipperLib clip fill type. \nSee https://goo.gl/5pYQQP", \
                            AREA_CLIPPER_FILL_SEQ, AREA_CLIPPER_FILL_INFO))

/** Deflection parameter */
#define AREA_PARAMS_DEFLECTION(_apply, _tail, ...) \
    _apply(__VA_ARGS__, FC_PP_IF(_tail, 1, 0), (double, deflection, Deflection, 0.01, \
...
#define AREA_PARAMS_BASE(_apply, _tail, ...) \
    ...
    _apply(__VA_ARGS__, 1, (enum, open_mode, OpenMode, 0, \
...
#define AREA_PARAMS_CAREA(_apply, _tail, ...) \
    _apply(__VA_ARGS__, FC_PP_IF(_tail, 1, 0), (double, tolerance, Tolerance, Precision::Confusion(), \
                                                "Point coincidence tolerance", App::PropertyPrecision)) \
    AREA_PARAMS_FIT_ARCS(_apply, 1, __VA_ARGS__) \
    _apply(__VA_ARGS__, 1, (bool, clipper_simple, Simplify, false, \
                            "Simplify polygons after operation. See https://goo.gl/Mh9XK1")) \
    _apply(__VA_ARGS__, 1, (double, clipper_clean_distance, CleanDistance, 0.0, \
                            "Clean polygon smaller than this distance. See https://goo.gl/jox3JY", \
                            App::PropertyLength)) \
    _apply(__VA_ARGS__, 1, (double, accuracy, Accuracy, 0.01, "Arc fitting accuracy", App::PropertyPrecision)) \
    _apply(__VA_ARGS__, 1, (double, units, Unit, 1.0, "Scaling factor for conversion to inch", App::PropertyFloat)) \
    _apply(__VA_ARGS__, 1, (short, min_arc_points, MinArcPoints, 4, "Minimum segments for arc discretization")) \
    _apply(__VA_ARGS__, 1, (short, max_arc_points, MaxArcPoints, 100, "Maximum segments for arc discretization (ignored currently)")) \
    _apply(__VA_ARGS__, 1, (double, clipper_scale, ClipperScale, 1e7, \
...
#define AREA_PARAMS_POCKET(_apply, _tail, ...) \
    _apply(__VA_ARGS__, FC_PP_IF(_tail, 1, 0), (enum, mode, PocketMode, 0, \
                                                "Selects the pocket toolpath pattern", \
                                                (None, ZigZag, Offset, Spiral, ZigZagOffset, Line, Grid, Triangle))) \
    _apply(__VA_ARGS__, 1, (double, tool_radius, ToolRadius, 1.0, "Tool radius for pocketing", App::PropertyLength)) \
    _apply(__VA_ARGS__, 1, (double, extra_offset, PocketExtraOffset, 0.0, "Extra offset for pocketing", \
                            App::PropertyDistance)) \
    _apply(__VA_ARGS__, 1, (double, stepover, PocketStepover, 0.0, \
                            "Cutter diameter to step over on each pass. If =0, use ToolRadius.", \
                            App::PropertyLength)) \
    _apply(__VA_ARGS__, 1, (double, last_stepover, PocketLastStepover, 0.0, \
    ...
    _apply(__VA_ARGS__, 1, (double, angle, Angle, 45, "Pattern angle in degree", App::PropertyAngle)) \
    _apply(__VA_ARGS__, 1, (double, angle_shift, AngleShift, 0.0, "Pattern angle shift for each section", \
                            App::PropertyAngle)) \
    _apply(__VA_ARGS__, 1, (double, shift, Shift, 0.0, \
...
#define AREA_PARAMS_POCKET_CONF(_apply, _tail, ...) \
    _apply(__VA_ARGS__, FC_PP_IF(_tail, 1, 0), (bool, thicken, Thicken, false, \
                                                "Thicken the resulting wires with ToolRadius"))

/** Operation code */
#define AREA_PARAMS_OPCODE(_apply, _tail, ...) \
    _apply(__VA_ARGS__, FC_PP_IF(_tail, 1, 0), (enum, op, Operation, 0, \
                                                "Boolean operation.\n" \
...
#define AREA_PARAMS_OFFSET(_apply, _tail, ...) \
    _apply(__VA_ARGS__, FC_PP_IF(_tail, 1, 0), (double, offset, Offset, 0.0, \
    ...
    _apply(__VA_ARGS__, 1, (double, stepover, Stepover, 0.0, \
                            "Cutter diameter to step over on each pass. If =0, use Offset", \
                            App::PropertyLength)) \
    _apply(__VA_ARGS__, 1, (double, last_stepover, LastStepover, 0.0, \
...
#define AREA_PARAMS_SECTION_EXTRA(_apply, _tail, ...) \
    _apply(__VA_ARGS__, FC_PP_IF(_tail, 1, 0), (enum, mode, SectionMode, 2, \
    ...
    _apply(__VA_ARGS__, 1, (bool, project, Project, false, \
...
#define AREA_PARAMS_SECTION(_apply, _tail, ...) \
    _apply(__VA_ARGS__, FC_PP_IF(_tail, 1, 0), (long, count, SectionCount, 0, \
                                                "Number of sections to generate. -1 means full sections.")) \
    _apply(__VA_ARGS__, 1, (double, stepdown, Stepdown, 1.0, \
                            "Step down distance for each section.\n" \
                            "Positive value means going from top down, and negative the other way round", \
                            App::PropertyDistance)) \
    _apply(__VA_ARGS__, 1, (double, offset, SectionOffset, 0.0, \
                            ...
                            App::PropertyDistance)) \
    _apply(__VA_ARGS__, 1, (double, tolerance, SectionTolerance, 1e-6, \
...
#define AREA_PARAMS_OFFSET_CONF(_apply, _tail, ...) \
    _apply(__VA_ARGS__, FC_PP_IF(_tail, 1, 0), (enum, algo, Algo, 0, "Offset algorithm type", (Clipper, libarea))) \
    _apply(__VA_ARGS__, 1, (enum2, join_type, JoinType, 0, \
                            "ClipperOffset join type. \nSee https://goo.gl/4odfQh", \
                            (Round, Square, Miter), \
                            (ClipperLib::JoinType, ClipperLib::jt))) \
    _apply(__VA_ARGS__, 1, (enum2, end_type, EndType, 0, \
    ...
    _apply(__VA_ARGS__, 1, (double, miter_limit, MiterLimit, 2.0, \
                            "Miter limit for joint type Miter. See https://goo.gl/K8xX9h", \
                            App::PropertyFloat)) \
    _apply(__VA_ARGS__, 1, (double, round_precision, RoundPrecision, 0.0, \
                            "Round joint precision. If =0, it defaults to Accuracy. \n" \
                            "See https://goo.gl/4odfQh", \
                            App::PropertyPrecision))
#else
#define AREA_PARAMS_OFFSET_CONF(_apply, _tail, ...) \
    _apply(__VA_ARGS__, FC_PP_IF(_tail, 1, 0), (enum2, join_type, JoinType, 0, \
                                                "ClipperOffset join type. \nSee https://goo.gl/4odfQh", \
                                                (Round, Square, Miter), \
                                                (ClipperLib::JoinType, ClipperLib::jt))) \
    _apply(__VA_ARGS__, 1, (enum2, end_type, EndType, 0, \
...
#define AREA_PARAMS_MIN_DIST(_apply, _tail, ...) \
    _apply(__VA_ARGS__, FC_PP_IF(_tail, 1, 0), (double, min_dist, MinDistance, 0.0, \
...
#define AREA_PARAMS_ARC_PLANE(_apply, _tail, ...) \
    _apply(__VA_ARGS__, FC_PP_IF(_tail, 1, 0), (enum, arc_plane, ArcPlane, 1, \
...
#define AREA_PARAMS_ORIENTATION(_apply, _tail, ...) \
    _apply(__VA_ARGS__, FC_PP_IF(_tail, 1, 0), (enum, orientation, Orientation, 0, \
...
#define AREA_PARAMS_SORT(_apply, _tail, ...) \
    _apply(__VA_ARGS__, FC_PP_IF(_tail, 1, 0), (enum, sort_mode, SortMode, 1, \
    ...
    _apply(__VA_ARGS__, 1, (double, threshold, RetractThreshold, 0.0, \
                            ...
                            App::PropertyLength)) \
    _apply(__VA_ARGS__, 1, (enum, retract_axis, RetractAxis, 2, "Tool retraction axis", (X, Y, Z)))
...
#define AREA_PARAMS_PATH(_apply, _tail, ...) \
    ...
    _apply(__VA_ARGS__, 1, (double, retraction, Retraction, 0.0, \
                            "Tool retraction absolute coordinate along retraction axis", \
                            App::PropertyLength)) \
    _apply(__VA_ARGS__, 1, (double, resume_height, ResumeHeight, 0.0, \
                            "When return from last retraction, this gives the pause height relative to the Z\n" \
                            "value of the next move.", \
                            App::PropertyLength)) \
    _apply(__VA_ARGS__, 1, (double, segmentation, Segmentation, 0.0, \
    ...
    _apply(__VA_ARGS__, 1, (double, feedrate_v, FeedRateVertical, 0.0, "Vertical only (step down) move feed rate", \
                            App::PropertyFloat)) \
    _apply(__VA_ARGS__, 1, (bool, verbose, Verbose, true, \
                            "If true, each motion GCode will contain full coordinate and feedrate")) \
    _apply(__VA_ARGS__, 1, (bool, abs_center, AbsoluteArcCenter, false, "Use absolute arc center mode (G90.1)")) \
