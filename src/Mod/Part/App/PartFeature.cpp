    ...
    if (res.second && !mapped.name) {
        ...
        if (dot) {
            ...
            if (occindex.second > 0) {
                ...
                const std::string_view indexPrefix{Data::POSTFIX_INDEX};
                if (ancestors.size() > 1 && std::string_view{postfix}.starts_with(indexPrefix)) {
                    std::istringstream iss(postfix.c_str() + indexPrefix.size());
                    int idx;
        ...
        if (pyobj && PyObject_TypeCheck(pyobj, &TopoShapePy::Type)) {
            ...
            if (linked->isDerivedFrom<App::Line>()) {
                ...
                if (element) {
                    ...
                    if (Base::StringViewTools::iequalsAscii(elementView, "x")
                        || Base::StringViewTools::iequalsAscii(elementView, "x-axis")
                        || Base::StringViewTools::iequalsAscii(elementView, "y")
                        || Base::StringViewTools::iequalsAscii(elementView, "y-axis")
                        || Base::StringViewTools::iequalsAscii(elementView, "z")
                        || Base::StringViewTools::iequalsAscii(elementView, "z-axis")) {
                        static TopoDS_Shape _shape;
                        if (_shape.IsNull()) {
                    ...
                    else if (Base::StringViewTools::iequalsAscii(elementView, "o")
                             || Base::StringViewTools::iequalsAscii(elementView, "origin")) {
                        static TopoDS_Shape _shape;
                        if (_shape.IsNull()) {
                            BRepBuilderAPI_MakeVertex builder(gp_Pnt(0, 0, 0));
                            _shape = builder.Shape();
