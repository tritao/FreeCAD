...
#ifndef PART_SPATIAL_INDEX_H
#define PART_SPATIAL_INDEX_H
...
public:
    ...
    std::optional<std::pair<Payload, double>> nearest(const gp_Pnt& point) const
    {
        ...
        struct QueueEntry
        {
            double dist2;
            int nodeId;
            bool operator>(const QueueEntry& other) const { return dist2 > other.dist2; }
        };
    ...
    std::vector<std::pair<Payload, double>> kNearest(const gp_Pnt& point, std::size_t k) const
    {
        ...
        struct BestEntry
        {
            double dist2;
            int nodeId;
            bool operator<(const BestEntry& other) const { return dist2 < other.dist2; }
        };
