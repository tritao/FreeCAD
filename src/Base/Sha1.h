...
{
inline std::uint32_t rotateLeft(std::uint32_t value, unsigned bits) noexcept
{
...
inline std::uint32_t loadBe32(const std::uint8_t* p) noexcept
{
    return (static_cast<std::uint32_t>(p[0]) << 24U)
        ...
        | (static_cast<std::uint32_t>(p[3]) << 0U);
}

inline void processBlock(const std::uint8_t block[64], std::uint32_t state[5]) noexcept
{
    std::uint32_t w[80] {};
    for (int i = 0; i < 16; ++i) {
...
inline std::array<std::uint8_t, 20> sha1Digest(BytesView bytes) noexcept
{
    ...
    while (offset + 64 <= size) {
        Sha1Detail::processBlock(data + offset, state);
        offset += 64;
    }
    ...
    Sha1Detail::processBlock(tail.data(), state);
    if (paddedSize == 128) {
        Sha1Detail::processBlock(tail.data() + 64, state);
    }
